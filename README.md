...
| [xiaoyao-cvs-plugin](https://github.com/ctrlcvs/xiaoyao-cvs-plugin) | 帮助、版本、体力、图鉴、地图位置、米游社绑定教程 |

鸣潮签到和开启自动签到属于写操作，只会给出确认，不会自动执行。没有进入
显式预设目录的命令仍可按原插件的准确命令使用，但不会由 AI 猜测后执行。
...
QQ 按钮确认功能取决于当前适配器是否支持 `segment.button`。不支持时会自动
退化为文字确认。
...
2. 没有插件处理时，aiEnhance-plugin 在本地召回少量相关候选；
3. 模型只能返回 `chat`、`clarify` 或某个已提供的 `candidateId`；
4. 本地代码校验参数并根据受限模板生成命令；
5. 安全策略检查风险、白名单、置信度、召回分数和目标插件规则；
6. 满足自动执行条件时，带防递归标记重新交给 Yunzai 调度器。
...
当前适配器可能不支持 QQ 按钮。插件会发送文字和准确命令，用户可以自行复制
或直接发送该命令。
